...
void WebCamWindow::trackHand()
{
    ...
    int match_method=3;
      ...
      rectangle( imgResult_, matchLoc, Point( matchLoc.x + templateWidth_ , matchLoc.y + templateHeight_ ), Scalar::all(0), 2, 8, 0 );

      imshow("result", imgResult_ );
      waitKey(10);

      //affichage
      qDebug() << "Position tracker :"<< matchLoc.x << ", " << matchLoc.y;
      //qDebug() << "Position sourie  :"<<cur->pos().rx()<<", "<< cur->pos().ry();

      //assocation tracker / sourie (si cohérant)
      int posCursX=matchLoc.x*screen->size().width()/result_cols;
      posCursX=screen->size().width()-posCursX;
      int posCursY=matchLoc.y*screen->size().height()/result_rows;
      cur->setPos(posCursX,posCursY);
